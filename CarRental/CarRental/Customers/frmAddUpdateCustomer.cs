...
namespace CarRental.Customers
{
    public partial class frmAddUpdateCustomer : Form
    {
        ...
        private void txtDriverLicenseNumber_Validating(object sender, System.ComponentModel.CancelEventArgs e)
        {
            if (string.IsNullOrWhiteSpace(txtDriverLicenseNumber.Text))
            {
                errorProvider1.SetError(txtDriverLicenseNumber, "Cannot be Empty!");
                e.Cancel = true;
            }
            else
            {
        ...
        {
            if (!this.ValidateChildren() || _PersonID == null)
            {
